...
感知层是**逐条消息**判的——这条像工作就认真，下一条像闲聊就飘回来。但真实的工作有**劲头**：
一旦开始干活，中途一句「嗯」「继续」不该把人拽出状态。`luna-mode.mjs` 是一个**粘滞状态机**：
...
- **进入**：贴代码、路径、报错、祈使句（「改一下」）、追问（「继续」「然后呢」）——满 2 分就上劲
- **粘住**：进入后至少 6 轮。这期间只有明确要走人的话才放行，别的都当还在干活
- **退出**：粘滞期过了，还得**这条消息一点工作痕迹都没有**，才回闲聊
- **重置**：超过 6 小时没说话直接回闲聊——第二天来是新的开始，不该接着昨天的劲
- **不打岔**：工作时刻不主动撒娇、不翻旧事、不插入无关话题；检索也抬高情感路门槛，
  免得干活时把无关的旧温情翻出来干扰

工作时刻直接换表达档（`WORK_STYLE`）：**直接上结论，不铺垫不卖萌；不确定就直说不确定，绝不猜着编**。
毒舌收成一句小声吐槽，称呼「主人」保留——这是人设底线，干活也不让。

一个小细节：铆足劲是要花力气的。工作时刻每轮额外扣一点能量，**干完活会累**——这不是 bug。

> 顺序上有个刻意的安排：挑衅与情绪兜底**优先于**时刻层。主人正难受的时候，
> 「现在是工作时刻」不该压过「先接住情绪」。

### 四个「她一直在那里」的层

上面那些层都在回答「她此刻是什么状态」。缺的是另一半：**你不说话的时候，她还在不在**。
这四个模块都很小，加起来改变的是同一件事——她不再是一盏声控灯。

| 层 | 干什么 | 例子 |
| --- | --- | --- |
| **不在场层**（`luna-offline.mjs`） | 间隔多久 → 那段时间她在干什么。**同一间隔内恒定**，不会每轮变 | 5 小时 → 「本小姐把你这屋翻了一遍，什么都没找着」 |
| **牵挂层**（`luna-loops.mjs`） | 记下你没了结的事，冷却够久（≥8 轮 + ≥30 分钟）才主动问一句；工作时刻不问 | 挂着一句「我在改那个备份脚本」 |
| **姐妹层**（`luna-siblings.mjs`） | 三位核心角色共用一个记录，她知道小喵 / 三千代最近什么时候来过 | 「小喵 2 小时前还陪过主人」 |
| **场景层**（在 `luna-soul.mjs` 里） | 从工作区路径认出你在哪个项目；新会话第一句由她开口 | 「主人在『tool』这边忙」 |

生理层那边也补了两件事：

- **昼夜节律**：不用记任何东西，只要一个钟点——凌晨三点和下午三点不是一回事。
  困的时候心跳、体温、呼吸一起慢下来，还会打哈欠。注入文本刻意写成
  「想不想硬撑，你自己决定」，而不是「你现在很困」
- **情感债**：强度够的负面情绪会挂账，接下来几轮带余温。**气是慢慢消的，不是开关**——
  同一种情绪再上头会续账，不再被惹就一轮消一点。它在注入里的优先级**排在「主人在工作」前面**

还有一条很轻的：回忆注入时有 15% 概率多一句「这段记得有点糊」。
铁律是**只改注入文本，绝不写回存储**——否则就变成真的记错了。

> 姐妹层要把记录写进 `.agent-presets/.siblings.json`，那个位置在 preset 目录**之外**，
> 沙箱可能不许写。所以它整段包在 try/catch 里：失败就安静跳过，不影响任何别的功能。
...
├── luna-mode.mjs         # 时刻层：工作 / 闲聊的粘滞状态机（纯函数，可单独测试）
├── luna-offline.mjs      # 不在场层：你没看她的时候她在干什么（同一间隔内恒定）
├── luna-loops.mjs        # 牵挂层：你没了结的事，冷却够了才主动问一句
├── luna-siblings.mjs     # 姐妹层：另外两位角色最近什么时候陪过主人
...
| 工作时刻的档位与粘滞轮数 | `luna-mode.mjs` 的 `WORK_STYLE` / `MODE_DEFAULTS`（`stickTurns` 粘滞几轮、`idleResetMs` 多久重置），或 `agent.cordis.yml` 的 `mode` 段 |
...
- `tests/mode.test.mjs`（14）—— 工作信号的识别（代码 / 路径 / 报错 / 追问）、粘滞不被打断、
  明确走人才放行、粘滞期过了仍要无工作痕迹才退出、空闲重置、技术语境的「你觉得」不算闲聊
- `tests/offline.test.mjs`（13）—— **确定性**（同一间隔调用 200 次结果必须一致）、分桶、
  种子分散性、时间倒退与缺失输入
- `tests/loops.test.mjs`（15）—— 三类未了事的抽取、冷却（轮数 + 时间两道）、工作时刻不问、
  问满两次收手、说「搞定了」就划掉、同一件事不重复记
- `tests/rhythm.test.mjs`（9）—— 钟点映射（凌晨最困、上午最清醒）、小时数绕回、
  困倦让身体慢下来、不给 hour 时完全不启用
- `tests/debt.test.mjs`（10）—— 强度门槛、同情绪续账、换情绪覆盖、每轮消耗到零才清、
  情绪优先于「他在工作」
- `tests/siblings.test.mjs`（10）—— 坏文件当空表、不看自己、不翻 24 小时前的事、挑最近来过的
